...
func main() {
	...
	reqA := &service.RequestA{Name: "serviceA"}
	resp, err := clientA.ChatA(context.Background(), reqA)
	if err != nil {
		log.Fatal(err)
	}
	log.Println(resp)
	...
	reqB := &service.RequestB{Name: "serviceB"}
	resp, err = clientB.ChatB(context.Background(), reqB)
	if err != nil {
		log.Fatal(err)
	}
	log.Println(resp)
